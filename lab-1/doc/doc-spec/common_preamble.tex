...
%-------------------------
\newcommand{\authorSIDheader}[1][white]{\begin{tabular}{c}\textcolor{#1}{\tiny\authorSID}\\[-6pt]\textcolor{#1}{\tiny\gitMark}\end{tabular}}
%-------------------------
...
%		pdfcopyright={Copyright © \Year, \Author. Все права защищены.},
	...
	pdfcaptionwriter={Prof., Dr.Tech.Sc. Alexandr P. Sokolov},
...
%----------------------------------------------------------
% подключение пакета для подсчета объектов
\usepackage{totcount}
...
\def\tempd{#4}
  ...
  \ifx\tempempty\tempb \def\tempb{Решение:}\fi
...
\newcommand{\FullOrganisationName}{федеральное государственное автономное образовательное учреждение высшего образования <<Московский государственный технический университет имени Н.Э.~Баумана (национальный исследовательский университет)>>}% (\ShortOrganisationName)}
...
\def\dfmn{д-р~физ.-мат.~наук}
\def\dhn{д-р~хим.~наук}
\def\dtn{д-р~техн.~наук}
\def\den{д-р~экон.~наук}
\def\Kfmn{Канд.~физ.-мат.~наук}
\def\Ktn{Канд.~техн.~наук}
\def\ktn{канд.~техн.~наук}
\def\kfmn{канд.~физ.-мат.~наук}
